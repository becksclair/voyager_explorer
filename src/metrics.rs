...
impl AppMetrics {
    ...
    pub fn new() -> Self {
        // Histogram for 1ms to 10 seconds with 2 significant digits
        let decode_latency_ms = Histogram::new_with_bounds(1, 10_000, 2).expect("Histogram creation should succeed");

        // Histogram for 1ms to 1 second with 2 significant digits
        let frame_time_ms = Histogram::new_with_bounds(1, 1_000, 2).expect("Histogram creation should succeed");
    ...
    pub fn record_decode(&mut self, duration: Duration, pixels: usize, success: bool) {
        ...
        if success {
            ...
            self.total_pixels_decoded.fetch_add(pixels as u64, Ordering::Relaxed);
        } else {
    ...
    pub fn set_worker_queue_depth(&self, depth: usize) {
        self.worker_queue_depth.store(depth as u64, Ordering::Relaxed);
    }
